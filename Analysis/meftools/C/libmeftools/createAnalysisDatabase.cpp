...
void createAnalysisDatabase( const char* name ) {
	...
	// analysis	dbIDanalysis,      name, description
    	...
    	dba.runSQL( queryStr );
    	dba.runSQL("create table analyses (uuid VARCHAR(36) PRIMARY KEY, name varchar(64), description varchar(128), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP );" );
	...
	// experiment	dbIDexperiment, name, dbIDcollection, dbIDanalysis
    	...
    	dba.runSQL( queryStr );
    	dba.runSQL("create table experiments (uuid VARCHAR(36) PRIMARY KEY, name varchar(64), dbIDcollection binary(16), dbIDanalysis binary(16), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP );" );
	...
	// events		dbIDevents,        name, dbIDexperiment, time, data, label
    	...
    	dba.runSQL( queryStr );
    	dba.runSQL("create table events (uuid VARCHAR(36) PRIMARY KEY, name varchar(64), dbIDexperiment binary(16), time BIGINT, data varchar(256), label varchar(32), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP );" );

	cout << "Create metrics table" << endl;
	// metrics		dbIDmetrics,       name, dbIDevents, value
    	sprintf( queryStr, "drop table if exists metrics;" );
    	dba.runSQL( queryStr );
    	dba.runSQL("create table metrics (uuid VARCHAR(36) PRIMARY KEY, name varchar(64), dbIDevent binary(16), value varchar(64), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP );" );
