...
namespace Rcpp {
  template <> SEXP wrap(const Rcpp::MEF_HEADER_INFO& x);
}
...
namespace Rcpp {
 template <>
 SEXP wrap(const Rcpp::MEF_HEADER_INFO& x) {
   std::vector<std::string> names;
   std::vector<SEXP> elements(47);
   // do something with the elements and names
   names.push_back("institution");
   elements[0] = wrap( x.institution );
   names.push_back("unencrypted_text_field");
   elements[1] = wrap( x.unencrypted_text_field );
   names.push_back("encryption_algorithm");
   elements[2] = wrap( x.encryption_algorithm );
   names.push_back("subject_encryption_used");
   elements[3] = wrap( x.subject_encryption_used );
   names.push_back("session_encryption_used");
   elements[4] = wrap( x.session_encryption_used );
   names.push_back("data_encryption_used");
   elements[5] = wrap( x.data_encryption_used );
   names.push_back("byte_order_code");
   elements[6] = wrap( x.byte_order_code );
   names.push_back("header_version_major");
   elements[7] = wrap( x.header_version_major );
   names.push_back("header_version_minor");
   elements[8] = wrap( x.header_version_minor );
   names.push_back("session_unique_ID");
   std::ostringstream buf;
   buf << (int)x.session_unique_ID[0];
   for (int j=1; j<8; j++) buf << " " << (int)( x.session_unique_ID[j] );
   elements[9] = wrap( buf.str() );
   names.push_back("header_length");
   elements[10] = wrap( x.header_length );
   names.push_back("subject_first_name");
   elements[11] = wrap( x.subject_first_name );
   names.push_back("subject_second_name");
   elements[12] = wrap( x.subject_second_name );
   names.push_back("subject_third_name");
   elements[13] = wrap( x.subject_third_name );
   names.push_back("subject_id");
   elements[14] = wrap( x.subject_id );
   names.push_back("session_password");
   elements[15] = wrap( x.session_password );
   names.push_back("subject_validation_field");
   elements[16] = wrap( x.subject_validation_field );
   names.push_back("session_validation_field");
   elements[17] = wrap( x.session_validation_field );
   names.push_back("protected_region");
   elements[18] = wrap( x.protected_region );
   names.push_back("number_of_samples");
   elements[19] = wrap( x.number_of_samples );
   names.push_back("channel_name");
   elements[20] = wrap( x.channel_name );
   names.push_back("recording_start_time");
   elements[21] = wrap( x.recording_start_time );
   names.push_back("recording_end_time");
   elements[22] = wrap( x.recording_end_time );
   names.push_back("sampling_frequency");
   elements[23] = wrap( x.sampling_frequency );
   names.push_back("low_frequency_filter_setting");
   elements[24] = wrap( x.low_frequency_filter_setting );
   names.push_back("high_frequency_filter_setting");
   elements[25] = wrap( x.high_frequency_filter_setting );
   names.push_back("notch_filter_frequency");
   elements[26] = wrap( x.notch_filter_frequency );
   names.push_back("voltage_conversion_factor");
   elements[27] = wrap( x.voltage_conversion_factor );
   names.push_back("acquisition_system");
   elements[28] = wrap( x.acquisition_system );
   names.push_back("channel_comments");
   elements[29] = wrap( x.channel_comments );
   names.push_back("study_comments");
   elements[30] = wrap( x.study_comments );
   names.push_back("physical_channel_number");
   elements[31] = wrap( x.physical_channel_number );
   names.push_back("compression_algorithm");
   elements[32] = wrap( x.compression_algorithm );
   names.push_back("maximum_compressed_block_size");
   elements[33] = wrap( x.maximum_compressed_block_size );
   names.push_back("maximum_block_length");
   elements[34] = wrap( x.maximum_block_length );
   names.push_back("block_interval");
   elements[35] = wrap( x.block_interval );
   names.push_back("maximum_data_value");
   elements[36] = wrap( x.maximum_data_value );
   names.push_back("minimum_data_value");
   elements[37] = wrap( x.minimum_data_value );
   names.push_back("index_data_offset");
   elements[38] = wrap( x.index_data_offset );
   names.push_back("number_of_index_entries");
   elements[39] = wrap( x.number_of_index_entries );
   names.push_back("block_header_length");
   elements[40] = wrap( x.block_header_length );
   names.push_back("GMT_offset");
   elements[41] = wrap( x.GMT_offset );
   names.push_back("discontinuity_data_offset");
   elements[42] = wrap( x.discontinuity_data_offset );
   names.push_back("number_of_discontinuity_entries");
   elements[43] = wrap( x.number_of_discontinuity_entries );
   names.push_back("file_unique_ID");
   buf.str("");
   buf.clear();
   buf << (int)x.file_unique_ID[0];
   for (int j=1; j<FILE_UNIQUE_ID_LENGTH; j++) buf << " " << (int)( x.file_unique_ID[j] );
   elements[44] = wrap( buf.str() );
   names.push_back("anonymized_subject_name");
   elements[45] = wrap( x.anonymized_subject_name );
   names.push_back("header_crc");
   elements[46] = wrap( x.header_crc );
/*
   names.push_back("file_index");
   elements[47] = wrap( x.file_index );
   names.push_back("discontinuity_data");
   elements[48] = wrap( x.discontinuity_data );
*/   
...
//
//  write_mef_ind() is identical to write_mef(), but also takes a pointer to the file index or table
...
#include <string.h>
// BEGIN --- AES_Encryption.h ---
#ifndef __AES_ENCRYPTION
#define __AES_ENCRYPTION
...
//
// Specification for Multiscale Electrophysiology Format (MEF) version 2.1
...
static ui4 update_crc_32(ui4 crc, si1 c ) {
...
static ui8 RED_compress_block(si4 *in_buffer, ui1 *out_buffer, ui4 num_entries, ui8 uUTC_time, ui1 discontinuity,
                             si1 *key, RED_BLOCK_HDR_INFO *block_hdr)
{
    ...
    *ui1_p1++ = discontinuity;
    ...
    if (*key)
        AES_encryptWithKey(ehbp, ehbp, (unsigned char*) key); //expanded key
...
ui8 RED_decompress_block(ui1 *in_buffer, si4 *out_buffer, si1 *diff_buffer, si1 *key, ui1 validate_CRC, RED_BLOCK_HDR_INFO *block_hdr_struct)
{
    ...
    *ui1_p++ = (*(si1 *)(ib_p-1)<0) ? -1 : 0; //sign extend
    ...
    if (validate_CRC && block_hdr_struct != NULL) {
    ...
    if (*key)
        AES_decryptWithKey(ib_p, ib_p, (unsigned char*) key); //pass in expanded key
    ...
    /*** range decode ***/
    diff_buffer[0] = -128; db_p = (ui1*) (diff_buffer + 1);	++diff_cnts;	// initial -128 not coded in encode (low frequency symbol)
...
static si4	read_mef_header_block(ui1 *header_block, Rcpp::MEF_HEADER_INFO *header_struct, si1 *password)
{
    ...
    sprintf(encrypted_string, "encrypted");
...
static void showHeader(Rcpp::MEF_HEADER_INFO *headerStruct)
{
    ...
    if (headerStruct->header_version_minor == 1)
    {
...
static si4	write_mef(si4 *samps, Rcpp::MEF_HEADER_INFO *mef_header, ui8 len, si1 *out_file, si1 *subject_password)
{
    ...
    if (mef_header->data_encryption_used)
        AES_KeyExpansion(4, 10, encryption_key, (unsigned char*) mef_header->session_password);
    
    
    index_block = (INDEX_DATA *)calloc(num_blocks, sizeof(INDEX_DATA));
    compressed_buffer = (si1*) calloc(num_blocks*samps_per_block, sizeof(si4)); //we'll assume at least 50% compression
    ...
    for (i=0; i<num_blocks; i++) {
        ...
        //printf("%ld ip %lu\t %lu\t %lu\t spb %u \n", i, ip->time, ip->file_offset, ip->sample_number, samps_per_block );
        
        RED_block_size = RED_compress_block(sp, (ui1*) cbp, samps_per_block, ip->time, (ui1)discontinuity_flag,
                                            (si1*) encryption_key, &RED_bk_hdr);
...
static si4	write_mef_ind(si4 *samps, Rcpp::MEF_HEADER_INFO *mef_header, ui8 len, si1 *out_file, si1 *subject_password, INDEX_DATA *index_block, si4 num_blocks, ui1 *discontinuity_array)
{
    ...
    compressed_buffer = (si1*) calloc(num_blocks*samps_per_block, sizeof(si4));
    ...
    for (i=0; i<num_blocks; i++) {
        ...
        RED_block_size = RED_compress_block(sp, (ui1*) cbp, samps_to_encode, ip->time, (ui1)discontinuity_array[i], (si1*) encryption_key, &RED_bk_hdr);
...
/*
  result(0) = header->institution;
  result(1) = header->unencrypted_text_field;
  result(2) = header->encryption_algorithm;
  result(3) = printf( "%u", header->subject_encryption_used );
  result(4) = printf( "%u", header->session_encryption_used );
  result(5) = printf( "%u", header->data_encryption_used );
  result(6) = printf( "%u", header->byte_order_code );
  char tmp [32];
  sprintf( tmp, "%u", header->header_version_major );
  std::string str(tmp);
  result(7) = tmp;
  result(8) = printf( "%u", header->header_version_minor );
  char buffer[128];
  int cnt = 0;
  sprintf(buffer, "%u", header->session_unique_ID[0]);
  for(i=1; i<SESSION_UNIQUE_ID_LENGTH; i++)
    sprintf(buffer, "%s %u", buffer, header->session_unique_ID[i]);
  std::string str_(buffer);
  result(9) = buffer;
  result(10) = header->header_length;
  result(11) = header->subject_first_name;
  result(12) = header->subject_second_name;
  result(13) = header->subject_third_name;
  result(14) = header->subject_id;
  result(15) = header->session_password;
  result(16) = header->subject_validation_field;
  result(17) = header->session_validation_field;
  result(18) = header->protected_region;
  std::ostringstream os;
  os.str(""); os.clear(); os << header->number_of_samples;
  result(19) = os.str();
  result(20) = header->channel_name;
  os.str(""); os.clear(); os << header->recording_start_time;
  result(21) = os.str();
  os.str(""); os.clear(); os << header->recording_end_time;
  result(22) = os.str();
  result(23) = header->sampling_frequency;
  result(24) = header->low_frequency_filter_setting;
  result(25) = header->high_frequency_filter_setting;
  result(26) = header->notch_filter_frequency;
  result(27) = header->voltage_conversion_factor;
  result(28) = header->acquisition_system;
  result(29) = header->channel_comments;
  result(30) = header->study_comments;
  result(31) = header->physical_channel_number;
  result(32) = header->compression_algorithm;
  os.str(""); os.clear(); os << header->maximum_compressed_block_size;
  result(33) = os.str();
  os.str(""); os.clear(); os << header->maximum_block_length;
  result(34) = os.str();
  os.str(""); os.clear(); os << header->block_interval;
  result(35) = os.str();
  result(36) = header->maximum_data_value;
  result(37) = header->minimum_data_value;
  os.str(""); os.clear(); os << header->index_data_offset;
  result(38) = os.str();
  os.str(""); os.clear(); os << header->number_of_index_entries;
  result(39) = os.str();
  result(40) = header->block_header_length;
  result(41) = header->GMT_offset;
  os.str(""); os.clear(); os << header->discontinuity_data_offset;
  result(42) = os.str();
  os.str(""); os.clear(); os << header->number_of_discontinuity_entries;
  result(43) = os.str();
  result(44) = header->file_unique_ID;
  result(45) = header->anonymized_subject_name;
  os.str(""); os.clear(); os << header->header_crc;
  result(46) = os.str();
  result(47) = header->file_index;
  result(48) = header->discontinuity_data;
*/
